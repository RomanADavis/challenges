...
ingredients.map! do |description|
  properties = description.split.map(&:chop)
  {capacity: properties[2].to_i, durability: properties[4].to_i,
    flavor: properties[6].to_i, texture: properties[8].to_i}
end


mixtures = ingredients.repeated_combination(100)

cookies = mixtures.map do |mixture|
  empty = {capacity: 0, durability: 0, flavor: 0, texture: 0}
  cookie = mixture.inject(empty) do |total, ingredient|
    total.keys.each {|property| total[property] += ingredient[property]}
    total
  end
  cookie.keys.each {|property| cookie[property] = 0 if cookie[property] < 0}
  cookie.values.inject(:*)
end
