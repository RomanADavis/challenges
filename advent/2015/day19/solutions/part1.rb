# udolph the Red-Nosed Reindeer is sick! His nose isn't shining very brightly, and he needs medicine.
#
...
molecule = File.read("./input/molecule.txt").chomp
transformations = File.read("./input/transformations.txt").split("\n")
...
class Machine
  def initialize(chemical, transformations)
    @chemical = chemical
  ...
  def transform
    @transformations.each do |tranformation|
      input = tranformation.keys.first
      output = tranformation.values.first
      @chemical.chars.each_cons(input.length).with_index do |(first, last), index|
        if [first, last].join == input
          form = @chemical.dup
          form[index..(index + 1)] = output if last
          form[index] = output unless last
          @forms << form
        end
      end
    end
...
puts Machine.new(molecule, replacements).transform.uniq.length
